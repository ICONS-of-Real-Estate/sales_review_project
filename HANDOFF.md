...
Kris's actual ask was "will we pick it up for the next training plan and their week of practicing" — that's Phase 5/6/7, all done above. The dashboard was **not** asked for and was deliberately left alone: `tools/dashboard/app.py`/templates don't surface `Flag: Framework Explained`/`Framework Gaps` at all, and `sync.py`'s `TRAINING_ASSIGNMENTS_COLUMNS` map was intentionally **not** extended to include the new "Training Framework (JSON)" column — doing that without also migrating the live `training_assignments` SQLite table on the VPS (`CREATE TABLE IF NOT EXISTS` is a no-op against an already-existing table) would have broken the *entire* sync run, not just that one tab, the next time `sync.py` executes on the VPS. Caught this before committing, not after — see the comment left in `sync.py` right above `TRAINING_ASSIGNMENTS_COLUMNS`. A real, bounded fast-follow if Kris wants it surfaced: a small schema migration (`ALTER TABLE training_assignments ADD COLUMN training_framework_json TEXT`, guarded against re-running on a fresh `CREATE TABLE` that already includes it) plus the `app.py`/template work, same shape as last session's `outcome_disposition` surfacing.
