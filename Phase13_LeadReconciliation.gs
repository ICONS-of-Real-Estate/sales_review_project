 ...
 *
 * READ-ONLY AUDIT. This file writes nothing — not to a sheet, not to GHL. It
 ...
 *   not_found  — searched GHL, nothing plausible came back. These are the
 *                ones to create as GHL contacts (Kris, 05/09/2026: "Add them
 *                to CRM"). Creating them is deliberately NOT done here —
 *                see the note on writes below.
 ...
 *
 * WHY NO WRITES HERE. Creating ~N contacts in a live CRM is exactly the kind
 * of thing that needs the log-and-revert treatment Phase 12 established
 * (GHL_MIGRATION_PLAN.md §8) plus a human reading the not_found list first —
 * a name-matching false negative would create a duplicate contact for a lead
 * that already exists, and duplicates in a CRM are much harder to clean up
 * afterwards than they are to avoid. Audit first, review the list, then a
...
function previewLeadReconciliation_() {
  ...
  if (notFound.length) {
    log_('');
    log_('NOT IN GHL — candidates to create as contacts:');
  ...
  log_('');
  log_('Nothing was written. Review the NOT IN GHL list above before anything creates contacts.');
  return results;
}
