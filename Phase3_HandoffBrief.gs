...
function buildHandoffBriefSystemPrompt_() {
  return [
    ...
    '',
    'Formatting: the rep reads this on their phone right before joining a call — a dense wall of text gets',
    'skimmed past. Any field with more than one distinct idea (a separate objection, a separate note, a',
    'separate fact) MUST put each idea on its own line, separated by a literal \\n — never chain multiple',
    'distinct ideas into one run-on sentence or paragraph. A field with only one idea can stay a single line.',
    '',
    ...
    '  "issues_and_goals": "string — 2-3 short lines (\\n-separated if more than one), each one pain point /',
    '   goal they actually stated — not one packed sentence",',
    '  "podcast_fit_angle": "string — 1-2 sentences: how the podcast offer was pitched, or should be,',
    '   to address what they said above",',
    '  "unresolved_objections": "string — one \\n-separated line per distinct objection raised on this call',
    '   that was NOT fully put to rest, so the next rep can pre-empt each one individually;',
    '   \\"None identified\\" if the prior call resolved everything",',
    ...
    '  "other_notes": "string — one \\n-separated line per distinct fact worth knowing before joining',
    '   (a rapport detail, a promise made, a scheduling quirk, a tone/personality note) — not all run',
    '   together; \\"None\\" if nothing stands out"',
    '}'
  ].join('\n');
}
